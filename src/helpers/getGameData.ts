...
const d2DataUrl = 'https://script.google.com/macros/s/AKfycbwYVxIgMyWA9WsvHen7C823cxOIkr5wiMX11IqgDzPcQOeb79dJ0g0jmlo_P1PuebQ1/exec'
...
type D2APIReply<GDO> = 
  GDO extends APIStoreNames ? D2APIReplyForOne<GDO> : 
  GDO extends APIStoreNames[] ? D2APIReplyForMany<GDO> :
...
async function getGameData<GDO extends APIStoreNames | APIStoreNames[] | 'lastUpdated'>(requestedObjects: GDO, freshness?: number | number[] ): Promise<D2APIReply<GDO>> { 
  ...
  if (freshness !== undefined) {
    ...
    if (freshness.length !== objectsToFetch.length) {
      throw new SyntaxError(`If provided, the length of 'freshness' must match the length of 'requestedObjects'.`)
    }
  }
  const responses = await Promise.all(objectsToFetch.map(async (obj, i) => {
    const fetchTime = Date.now()
    console.log(`       Fetching ${obj}...`)
    const raw = await fetch(`${d2DataUrl}?obj=${obj}${freshness !== undefined ? `&freshness=${(freshness as number[])[i]}` : ''}`)
    ...
    console.log(`       ...${Date.now() - fetchTime}ms to retrieve ${obj} (${response.time}ms API generation)`)
